    ...
    >
      ...
      <div class="relative w-full overflow-hidden bg-zinc-900" [style.paddingBottom.%]="125">
        ...
        <div class="absolute top-2 left-2 right-2 flex items-start justify-between">
          ...
          @if (item().redditScore) {
            <span
              class="rounded-full bg-orange-900/60 px-2 py-0.5 text-[10px] font-mono text-orange-300"
      ...
      <div class="flex flex-col gap-1.5 p-3">
        <p class="text-xs font-semibold text-slate-100 line-clamp-2 leading-snug">
          {{ item().title }}
        </p>
        ...
        @if (item().tags.length > 0) {
          <div class="flex flex-wrap gap-1">
            @for (tag of item().tags.slice(0, 3); track tag) {
              <span class="rounded bg-zinc-800 px-1.5 py-0.5 text-[9px] text-slate-400">{{
                tag
              }}</span>
            }
          </div>
