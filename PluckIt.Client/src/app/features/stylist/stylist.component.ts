...
import { ChatService, ChatEvent } from '../../core/services/chat.service';
...
@Component({
  ...
  imports: [FormsModule],
  template: `
    <div class="flex flex-col h-full bg-background-dark text-chrome">
      <!-- Panel header -->
      <div
        class="h-16 flex items-center justify-between px-6 border-b border-border-subtle bg-card-dark/60 shrink-0"
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-chrome font-semibold text-sm">AI Stylist</h3>
        ...
        <div class="flex items-center gap-1">
          <!-- Memory button -->
          <button
            class="text-slate-text hover:text-chrome transition-colors p-1.5 rounded"
          ...
          <button
            class="lg:hidden text-slate-text hover:text-chrome transition-colors p-1"
            (click)="closed.emit()"
      ...
      @if (debugEnabled()) {
        <div
          class="border-b border-primary/40 bg-app-elevated px-4 py-2 text-[11px] text-slate-300 font-mono"
        >
          ...
          <div class="mt-2 max-h-24 overflow-auto">
            <pre class="text-[10px] leading-snug text-app-soft whitespace-pre-wrap">{{
              debugEvents().join(
                '
'
              )
      ...
      @if (memoryOpen()) {
        <div class="border-b border-border-subtle bg-app-elevated px-4 py-3 shrink-0">
          <p class="text-[10px] text-slate-500 font-mono uppercase tracking-widest mb-2">
            Conversation Memory (editable)
          </p>
          <textarea
            class="w-full bg-background-dark border border-border-chrome text-slate-300 text-xs font-mono p-2 resize-none focus:outline-none focus:border-primary rounded"
            rows="4"
            [(ngModel)]="memoryDraft"
          ...
          <div class="flex gap-2 mt-1.5">
            ...
            <button
              class="text-[10px] text-slate-500 hover:text-chrome font-mono uppercase"
      ...
      @if (selectedItemIds() && selectedItemIds()!.length > 0) {
        ...
        >
          ...
          >
          <button class="ml-auto text-slate-500 hover:text-chrome" (click)="clearSelected.emit()">
          ...
          @if (msg.role === 'assistant') {
            <div class="flex gap-3">
              ...
              <div class="flex flex-col gap-1 min-w-0">
                ...
                <div
                  class="bg-bg-soft p-3 rounded-2xl rounded-tl-none text-sm text-chrome leading-relaxed border border-border-chrome whitespace-pre-wrap"
                >
                  {{ msg.text
                  }}<span
                    [class]="
            ...
            <div class="flex gap-3 flex-row-reverse">
              ...
              <div class="flex flex-col gap-1 items-end min-w-0 max-w-[82%]">
                <span class="text-[11px] text-slate-400 font-mono">{{ msg.time }}</span>
                <div
                  class="bg-primary/85 border border-primary/40 shadow-[0_8px_24px_rgba(196,131,106,0.35)] p-3 rounded-2xl rounded-tr-none text-sm text-white leading-relaxed whitespace-pre-wrap"
                >
                  {{ msg.text }}
                </div>
              </div>
          ...
          <div class="flex gap-3">
            ...
            <div class="flex flex-col gap-2">
              @if (!currentTool() && !streamingActive()) {
                <div
                  class="bg-bg-soft px-4 py-3 rounded-2xl rounded-tl-none border border-border-chrome flex gap-1.5 items-center"
                >
      ...
      <div class="p-4 border-t border-border-subtle bg-card-dark shrink-0">
        <div class="relative">
          <input
            class="w-full bg-background-dark border border-border-chrome text-chrome rounded-lg pl-4 pr-12 py-3 text-sm focus:ring-1 focus:ring-primary focus:border-primary placeholder-slate-500 outline-none transition-colors"
          ...
          />
          <button
            class="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-primary rounded-md text-chrome hover:bg-primary/80 transition-colors disabled:opacity-40"
            (click)="sendMessage()"
...
export class StylistPanelComponent implements OnInit, OnDestroy {
  ...
  private readonly session = inject(StylistSessionService);
  ...
  ngOnInit(): void {
    ...
    this.session.ensureGreeting(this.now());
  ...
  sendMessage(): void {
    ...
    this.streamSub = this.chat
      .streamMessage(text, this.session.chatHistory(), this.selectedItemIds() ?? undefined)
      .subscribe({
  ...
  private handleEvent(event: ChatEvent, userText: string): void {
    ...
    switch (event.type) {
      ...
      case 'done': {
        ...
        if (finalMsg) {
          this.session.chatHistory.set([
            ...this.session.chatHistory().slice(-14), // keep last 7 exchanges (14 messages)
