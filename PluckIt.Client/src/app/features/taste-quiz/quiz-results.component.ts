...
@Component({
  ...
  styles: [`
    ...
    .fade-up { animation: fade-up 0.5s cubic-bezier(0.23,1,0.32,1) both; }
  `],
  template: `
    <div class="flex flex-col items-center justify-start min-h-screen bg-black text-slate-100 px-6 py-10 gap-8 fade-up">
      ...
      @if (profile.styleKeywords.length > 0) {
        <div class="w-full max-w-sm space-y-3">
          <p class="text-[10px] font-bold uppercase tracking-widest text-slate-500 text-center">Style Signals</p>
      ...
      @if (profile.brands.length > 0) {
        <div class="w-full max-w-sm space-y-3">
          <p class="text-[10px] font-bold uppercase tracking-widest text-slate-500 text-center">Favourite Brands</p>
          <div class="flex flex-wrap justify-center gap-2">
            @for (brand of profile.brands; track brand) {
              <span class="rounded-lg bg-zinc-900 border border-border-chrome px-3 py-1.5 text-xs text-slate-300">
                {{ brand }}
              </span>
            }
          </div>
      ...
      <!-- Inferred from badge -->
      <div class="flex items-center gap-2 rounded-full bg-zinc-900 border border-border-chrome px-4 py-2">
        <span class="material-symbols-outlined text-sm text-slate-500">
          {{ profile.inferredFrom === 'mood_cards' ? 'style' : 'image' }}
        </span>
        <span class="text-[10px] text-slate-500">
          Inferred from {{ profile.inferredFrom === 'mood_cards' ? 'mood archetypes' : 'outfit images' }}
        </span>
      </div>
